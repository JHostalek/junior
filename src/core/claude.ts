...
export async function extractHook(input: string): Promise<ExtractedHook> {
  const prompt = [
    ...
    '- "checkFn": a JavaScript function body (NOT a full function declaration) that will be executed in a sandboxed Worker with a `ctx` object.',
    '  The ctx object has these methods:',
    '    ctx.git(...args: string[]): Promise<string> — run read-only git commands and return stdout.',
    '      Allowed subcommands: status, log, diff, show, rev-parse, branch, tag, ls-files, ls-tree, cat-file, remote, rev-list, describe, name-rev, shortlog, blame, count-objects, for-each-ref, merge-base.',
    '      Write commands (push, checkout, reset, clean, etc.) are blocked.',
    '    ctx.readFile(path: string): Promise<string> — read a file within the repository',
    '    ctx.exec(cmd: string, args: string[]): Promise<string> — run a shell command and return stdout (may be restricted by config)',
